<?php
$title ="Success";
require_once "includes/header.php";
require_once 'db/conn.php';

if(isset($_POST['submit'])){
  // extract values from the _POST array
  $fname = $_POST['firstname'];
  $lname = $_POST['lastname'];
  $dob = $_POST['dob'];
  $email = $_POST['email'];
  $contact = $_POST['phone'];
  $specialty = $_POST['specialty'];
// call function to track if success or not
  $issuccess = $crud->insertAttendees($fname, $lname, $dob, $email, $contact, $specialty);

  if($issuccess){
    // echo '<h1 class ="text-center text-success">You Have been registred</h1>';
    include 'includes/successmessage.php';


  }
  else{
    // echo '<h1 class ="text-center text-danger">There was an error in processing</h1>';
    include 'includes/errormessage.php';



  }
}

?>
<!-- <h1 class ="text-center text-success">You Have beeen registred</h1> -->
<!-- This print GET values -->
<!-- <div class="card" style="width: 18rem;">
  <div class="card-body">
    <h5 class="card-title"><?php  echo $_GET["firstname"]. ' '. $_GET["firstname"];  ?></h5>
    <h6 class="card-subtitle mb-2 text-muted">

    <?php  echo $_GET["speciallity"];  ?>
    </h6>
    <p class="card-text">Date of Birth: <?php  echo $_GET["dob"];  ?> </p>
    <p class="card-text">Contact : <?php  echo $_GET["phone"];  ?> </p>
    <p class="card-text">Email: <?php  echo $_GET["email"];  ?> </p>
    
  </div>
</div> -->

<div class="card" style="width: 18rem;">
  <div class="card-body">
    <h5 class="card-title"><?php  echo $_POST["firstname"]. ' '. $_POST["lastname"];  ?></h5>
    <h6 class="card-subtitle mb-2 text-muted">

    <?php  echo $_POST["specialty"];  ?>
    </h6>
    <p class="card-text">Date of Birth: <?php  echo $_POST["dob"];  ?> </p>
    <p class="card-text">Contact : <?php  echo $_POST["phone"];  ?> </p>
    <p class="card-text">Email: <?php  echo $_POST["email"];  ?> </p>
    
  </div>
</div>




<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<?php require_once "includes/footer.php";  ?>